    ...
    >
      {showVideo && stream ? (
        ...
        <div className="flex h-full w-full flex-col items-center justify-center gap-3 text-center">
          <span className="flex h-16 w-16 items-center justify-center rounded-full bg-surface text-2xl font-bold text-muted sm:h-20 sm:w-20">
            {label.slice(0, 1)}
          </span>
          <span className="text-xs font-semibold tracking-widest text-muted sm:text-sm">
            {placeholderText}
          </span>
        </div>
      )}

      <div className="absolute left-3 top-3 flex max-w-[82%] flex-col items-start gap-2 sm:left-4 sm:top-4">
        {topTokenReady &&
          (topToken ? (
            <div className="flex flex-col gap-1 rounded-2xl border border-white/10 bg-black/75 px-3.5 py-2.5 backdrop-blur-md sm:px-5 sm:py-3.5">
              <span className="text-[9px] font-bold tracking-[0.18em] text-muted sm:text-[11px]">
                MOST PROFITABLE
              </span>
              <span className="flex min-w-0 items-center gap-2">
                {topToken.imageUrl ? (
                  ...
                  <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-accent/20 text-xs sm:h-8 sm:w-8 sm:text-base">
                    🪙
                  </span>
                )}
                <span className="truncate text-xl font-extrabold leading-none text-white sm:text-2xl md:text-3xl">
                  {topToken.symbol}
                </span>
              </span>
              <span className="flex flex-wrap items-baseline gap-x-3 gap-y-0.5">
                <span
                  className={`text-base font-extrabold sm:text-lg md:text-xl ${topToken.pnl >= 0 ? "text-accent" : "text-loss"}`}
                >
                  {formatPnl(topToken.pnl)}
                </span>
                <span
                  className={`text-xs font-bold sm:text-sm md:text-base ${topToken.pnlPercent >= 0 ? "text-accent/80" : "text-loss/80"}`}
                >
            ...
            <div className="flex flex-col gap-1 rounded-2xl border border-white/10 bg-black/75 px-3.5 py-2.5 backdrop-blur-md sm:px-5 sm:py-3">
              <span className="text-[9px] font-bold tracking-[0.18em] text-muted sm:text-[11px]">
                TOP TRADE
              </span>
              <span className="text-sm font-bold text-muted sm:text-base">NO DATA</span>
            </div>
          ))}
        <span className="rounded-full bg-black/60 px-3 py-1.5 text-xs font-semibold text-white backdrop-blur-sm sm:text-sm">
          {label}
        </span>
      </div>
      ...
      {needsAudioTap && (
        <button
          ...
          className="absolute right-4 top-4 rounded-full bg-loss/90 px-3 py-1.5 text-xs font-bold tracking-wide text-white backdrop-blur-sm"
        >
          TAP FOR SOUND
        </button>
      )}
      ...
      {statusText && (
        <div className="absolute inset-x-0 bottom-0 bg-black/60 px-3 py-2.5 text-center text-xs font-semibold tracking-wide text-white backdrop-blur-sm sm:text-sm">
          {statusText}
        </div>
      )}
    </div>
